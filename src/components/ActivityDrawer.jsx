...
export default function ActivityDrawer({ isOpen, onOpenChange }) {
  return (
    <>
      {/* Trigger button */}
      <motion.button
      ...
      {/* Backdrop — only covers content below nav */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
      ...
      {/* Drawer — starts below the sticky nav (top-14 = 56px) */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
            transition={{ type: 'spring', stiffness: 300, damping: 30 }}
            className="fixed right-0 bottom-0 w-full max-w-sm z-50 flex flex-col border-l border-purple-800/50 shadow-2xl"
