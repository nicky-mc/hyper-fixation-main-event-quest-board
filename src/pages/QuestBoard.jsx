...
import { Plus, Scroll, Crown, Swords, Loader2, Fish, Radio, Star, Settings, ArrowUpDown, Map, Activity, Rss, Send, Trash2, Music, Video, Image as ImageIcon, MessageCircle, ChevronDown, ChevronUp, Bookmark, CheckCircle2, Clock, Shield, Zap, Trophy, Flame, Users, ChevronRight } from 'lucide-react';
...
// Local Component Imports (Ensure these paths are correct in your repo)
...
const particles = Array.from({ length: 8 }, (_, i) => ({
  ...
  style: {
    ...
    background: [
      '#a855f7', '#f59e0b', '#ef4444', '#06b6d4', '#22c55e'
...
export default function QuestBoard() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Auth call after initial data burst
  ...
  CATEGORIES.forEach(cat => {
    if (cat.id !== 'all') {
      categoryCounts[cat.id] = filterQuestsByCategory(pendingQuests, cat.id).length;
    }
  });
  ...
  // The "Roll for Initiative" Engine
  const rollForInitiative = async () => {
    ...
    const timer = setInterval(() => {
      ...
      if (elapsed >= rollDuration) {
        ...
        // A11y Announcement
        setSrAnnouncement(`${txt.rollBtn}: "${selected.title}" has been selected for the next episode!`);
        
        // Status Update & Global Log Entry
  ...
  return (
    <div className="min-h-screen w-full relative overflow-hidden max-w-[100vw] bg-transparent">
      ...
      {/* Optimized Background Layer: Multi-layered radial gradients */}
      <div 
        className="absolute inset-0 h-full pointer-events-none overflow-hidden transform-gpu translate-z-0" 
        style={{ backfaceVisibility: 'hidden' }}
      >
        <div 
          className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-[500px] opacity-20 rounded-full transform-gpu"
          style={{ 
        ...
        />
        <div 
          className="absolute bottom-0 left-0 w-[400px] h-[400px] opacity-20 rounded-full transform-gpu"
          style={{ 
      ...
      {/* Side Decorative Bars: Preservation of LCARS high-fidelity lines */}
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        >
          ...
          <div className="flex justify-center mb-4">
            <img 
              ...
              style={{ 
                ...
                boxShadow: '0 0 40px rgba(251,191,36,0.3), 0 0 80px rgba(239,68,68,0.15)' 
              }} 
            />
          </div>
          ...
          <div className="flex flex-col items-center justify-center mb-6">
            ...
            <div className="flex items-center justify-center gap-4 w-full max-w-lg mt-2">
              ...
              />
              <span 
                className="font-black tracking-[0.2em] uppercase text-red-500 leading-none"
                style={{ 
          ...
          <p className="text-slate-300 max-w-2xl mx-auto text-sm sm:text-base mt-2 leading-relaxed">
            ...
            We aren't experts, we are just two adventurers exploring the lore of life, pop culture, and deep dives. Submit your topics below, and we'll <span className="text-amber-400 font-semibold">Roll for Initiative</span> to pick our next discussion!
          </p>

          {/* Host Identifier Cards */}
          <div className="flex items-center justify-center gap-4 mt-5 flex-wrap">
            {[
              ...
              <div key={h.name} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/5 border border-white/10 shadow-inner">
        ...
        >
          {/* Post a Quest Primary Action */}
          <motion.button
            whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
            onClick={() => user ? setIsDrawerOpen(true) : base44.auth.redirectToLogin(window.location.pathname)}
            className="font-lcars h-11 flex items-center gap-2 px-5 rounded-xl border-2 border-purple-600/50 bg-gradient-to-r from-purple-900/80 to-indigo-900/80 hover:from-purple-800 hover:to-indigo-800 hover:border-purple-500 text-purple-200 font-bold text-sm shadow-lg"
          >
          ...
          {isAdmin && (
            <motion.button
              ...
              className={cn(
                "font-lcars h-11 flex items-center gap-2 px-5 rounded-xl border-2 font-bold text-sm transition-all duration-300",
                sortByVotes 
                  ? "bg-[var(--accent)]/20 border-[var(--accent)] text-[var(--accent)] shadow-[0_0_15px_var(--accent)]" 
          ...
          {isAdmin && (
            <motion.button
              ...
              className="font-lcars h-11 flex items-center gap-2 px-5 rounded-xl border-2 border-slate-600/50 bg-slate-900/40 text-slate-300 hover:border-slate-400 hover:bg-slate-800/60 font-bold text-sm tracking-widest uppercase transition-all duration-300"
          ...
          {/* Quest Map: High-Fidelity Navigation restoration */}
          <motion.button
            whileHover={{ scale: 1.04 }} 
            whileTap={{ scale: 0.96 }}
            onClick={() => setMapOpen(true)}
            className="font-lcars h-11 flex items-center gap-2 px-5 rounded-xl border-2 border-cyan-800/50 bg-cyan-950/30 text-cyan-400 hover:border-cyan-600/70 font-bold text-sm transition-all duration-300 shadow-[0_0_10px_rgba(6,182,212,0.1)]"
          >
            <Map className="w-4 h-4" /> {txt.mapBtn}
          </motion.button>

          {/* Live Feed: Sub-Space access */}
          <ActivityDrawer 
            isOpen={feedOpen} 
        ...
        {/* ── QUEST GRID: Logic and Fidelity Restoration ── */}
        <div className="flex flex-col gap-6">
          <div className="flex-1 min-w-0">
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-24 gap-4">
                ...
                <motion.span 
                  className="text-purple-400/70 font-mono text-sm tracking-widest" 
                  initial={{ opacity: 0 }} 
                  animate={{ opacity: 1 }}
                >
              ...
              >
                <div className="w-24 h-24 rounded-full bg-purple-950/30 border-2 border-purple-800/30 flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(168,85,247,0.2)]">
                  <Fish className="w-10 h-10 text-cyan-700/50" />
                </div>
                <h3 className="text-3xl text-purple-300/70 mb-2 font-display">
                  {txt.noQuests}
                </h3>
              ...
              >
                <p className="text-3xl text-purple-300/50 mb-2 font-display">No quests in this zone yet...</p>
          ...
          </div>{/* end flex-1 grid inner */}
        </div>{/* end quest grid outer */}

        {/* ── SELECTED ANNOUNCEMENT: Restoration of Starfleet Crown ── */}
        <AnimatePresence>
          {selectedQuestId && !isRolling && (
            ...
            >
              <div 
                className="inline-flex items-center gap-3 px-6 py-3 rounded-xl border border-amber-500/50"
                style={{ background: 'linear-gradient(135deg, rgba(120,53,15,0.3), rgba(13,8,32,0.8))', boxShadow: '0 0 30px rgba(251,191,36,0.2)' }}
              >
                <Crown className="w-6 h-6 text-amber-400" />
                <span className="text-xl text-amber-200 font-bold font-display">
                  {txt.selectedMsg}
                </span>
                <Crown className="w-6 h-6 text-amber-400" />
              </div>
        ...
        {/* ── SEGMENT LEGEND: Preservation of full segment mapping ── */}
        <motion.div 
          initial={{ opacity: 0 }} 
          animate={{ opacity: 1 }} 
          transition={{ delay: 0.5 }}
          className="mt-16 border border-purple-900/40 rounded-xl p-5 bg-white/[0.02] backdrop-blur-sm"
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 text-[10px]">
            {[
              ...
              <div key={name} className="flex flex-col gap-0.5 px-2 py-1.5 rounded bg-white/[0.03] border border-white/5 hover:border-purple-500/30 transition-colors">
        ...
        <div className="mt-10 text-center">
          <span className="text-slate-700 text-xs font-mono tracking-widest uppercase">
            ⚔️ THE HYPER-FIXATION MAIN EVENT · CO-OP MODE ACTIVE · 🦈 SHARKS DETECTED 🦈
          </span>
        </div>
      </div>{/* end max-w-7xl relative container */}

      <QuestSubmissionDrawer 
        isOpen={isDrawerOpen} 
        onClose={() => setIsDrawerOpen(false)} 
        onQuestSubmitted={loadQuests} 
      />
      <HostSettingsModal 
        isOpen={settingsOpen} 
      ...
      <AnimatePresence>
        {mapOpen && (
          <QuestWorldMap 
            quests={quests} 
            onClose={() => { setMapOpen(false); setTargetQuest(null); }} 
